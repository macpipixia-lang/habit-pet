  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
      <Card className="overflow-hidden p-0">
        ...
        <div className="grid gap-3 px-6 py-5 sm:grid-cols-2">
          {zhCN.landing.highlights.map((item) => (
            <div key={item} className="rounded-2xl border border-line bg-panelAlt/70 p-4 text-sm text-mist">
              {item}
            </div>
          ))}
        </div>
      ...
      <Card className="flex flex-col justify-between">
        ...
        <div className="mt-8 rounded-3xl border border-accent/25 bg-gradient-to-br from-accent/18 to-accentWarm/10 p-5">
          <p className="text-sm uppercase tracking-[0.18em] text-accent">{zhCN.landing.designBadge}</p>
          <p className="mt-3 text-sm leading-7 text-ink/80">{zhCN.landing.designDescription}</p>
        </div>
      </Card>
    </div>
  );
