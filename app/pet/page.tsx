...
function getPetStage(level: number) {
  if (level >= 25) return "Aurora Guardian";
  if (level >= 18) return "Moonlit Companion";
  if (level >= 10) return "Cloud Hopper";
  if (level >= 4) return "Sprout Cub";
  return "Tiny Egg";
}
    ...
    <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <Card className="overflow-hidden p-0">
        <div className="border-b border-line bg-gradient-to-br from-sky-400/20 via-cyan-300/8 to-transparent px-6 py-6">
          <Pill className="text-accent">Pet panel</Pill>
          <p className="mt-4 text-sm uppercase tracking-[0.2em] text-mist">{getPetStage(profile.level)}</p>
          <h1 className="mt-2 text-4xl font-semibold text-white">Level {profile.level}</h1>
          <p className="mt-3 text-sm leading-7 text-mist">Your pet keeps eating EXP even after level {MAX_LEVEL}. Only level growth stops.</p>
        </div>
        <div className="px-6 py-6">
          <div className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(125,211,252,0.3),_transparent_45%),linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-8 text-center">
            ...
            <p className="mt-2 text-sm text-mist">{formatNumber(profile.exp)} EXP absorbed</p>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-6">
        <Card>
          <Pill className="text-accentWarm">Progression</Pill>
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            <div>
              <p className="text-sm text-mist">EXP</p>
            ...
            <div>
              <p className="text-sm text-mist">Streak</p>
          ...
          <p className="mt-3 text-sm text-mist">{profile.level >= MAX_LEVEL ? "Max level reached." : `${profile.exp % EXP_PER_LEVEL} / ${EXP_PER_LEVEL} EXP to the next level.`}</p>
        </Card>

        <Card>
          <Pill className="text-accent">Milestones</Pill>
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {[1, 5, 10, 20, 30].map((milestone) => (
              <div key={milestone} className="rounded-2xl border border-line bg-black/20 p-4">
                <p className="text-sm text-mist">Level {milestone}</p>
                <p className="mt-2 text-base text-white">{profile.level >= milestone ? "Reached" : "Locked"}</p>
              </div>
            ))}
          </div>
        </Card>
